Zenith Finance is a web-only MVP for reviewing transactions, managing budgets, ingesting drafts, and surfacing actionable insights with multi-currency support. The app runs on Next.js 15, React 19, and Tailwind CSS v4 using the App Router. A detailed product requirements document lives at [`docs/zenith-finance-prd.md`](docs/zenith-finance-prd.md).
...
Add end-to-end coverage under `apps/web/tests` using Playwright.

Run the skeleton suite (currently all skipped) with:

```bash
npm run test:e2e -- --list
```

Example areas for future scenarios include transaction filters/CSV export, budget recalculation flows, draft ingestion actions, and scenario planning deltas.
