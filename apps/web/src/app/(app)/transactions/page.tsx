...
import { getCategoryHintContext, getTransactions, summarizeTransactions } from "@/server/queries";
...
export default async function TransactionsPage({
  ...
  const [transactions, metrics, hintContext] = await Promise.all([
    getTransactions({
    ...
    getCategoryHintContext(),
  ]);
  const allTransactions = hintContext.transactions;
  const learnedHints = hintContext.hints;
          ...
          <table className="w-full min-w-full divide-y divide-border/60 text-left">
            ...
            <tbody className="divide-y divide-border/60 text-sm">
              {transactions.map((txn) => {
                const suggestions = suggestCategories(txn.payee, {
